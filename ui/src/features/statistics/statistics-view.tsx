  ...
  return (
    <section className="h-full overflow-y-auto bg-[var(--canvas)]">
      <div className="min-h-full w-full px-[clamp(24px,4vw,64px)] py-[clamp(24px,4vh,44px)]">
        ...
        <div className="mt-8 grid grid-cols-4 overflow-hidden rounded-[15px] border border-[var(--line)] bg-white">
          {[
            ...
            <div key={label} className={`px-5 py-4 ${index > 0 ? "border-l border-[var(--line)]" : ""}`}>
        ...
        <div className="mt-5 grid grid-cols-[minmax(0,1.5fr)_minmax(250px,.5fr)] gap-5">
          <div className="rounded-[15px] border border-[var(--line)] bg-white p-5">
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                ...
                <p className="mt-1 text-[9px] text-[var(--muted-light)]">{t("stats.displayOnly")}</p>
              </div>
            </div>
            <div className="relative mt-7 flex h-[154px] items-end gap-2 border-b border-black/[.07] px-2">
              ...
              {chart.length > 0 ? (
                ...
                <div className="mb-12 w-full text-center text-[10px] text-[var(--muted-light)]">
                  {t("stats.emptyChart")}
                </div>
              )}
            </div>
            <div className="mt-2 flex justify-between text-[9px] text-[var(--muted-light)]">
          ...
          <div className="rounded-[15px] border border-[var(--line)] bg-white p-5">
            ...
            <dl className="mt-6 border-t border-[var(--line)] pt-4 text-[10px]">
              <div className="flex justify-between">
                <dt className="text-[var(--muted-light)]">{t("stats.largestWin")}</dt>
                <dd className="font-medium tabular-nums">{formatTokens(metrics.largestWin)}</dd>
              </div>
              <div className="mt-2.5 flex justify-between">
                <dt className="text-[var(--muted-light)]">{t("stats.largestLoss")}</dt>
        ...
        <div className="mt-5 overflow-hidden rounded-[15px] border border-[var(--line)] bg-white">
          <div className="flex items-center justify-between border-b border-[var(--line)] px-5 py-4">
            <div>
              ...
              <p className="mt-1 text-[9px] text-[var(--muted-light)]">{t("stats.recentDescription")}</p>
            </div>
            <FileCheck2 className="size-4 text-[var(--muted-light)]" strokeWidth={1.7} />
          </div>
          {metrics.recentHands.length > 0 ? (
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-[var(--line)] text-[9px] font-medium text-[var(--muted-light)]">
                ...
                {metrics.recentHands.map((hand) => {
                  ...
                  return (
                    <tr key={hand.address} className="border-b border-black/[.045] last:border-b-0 hover:bg-black/[.018]">
                      <td className="px-5 py-3 text-[11px] font-medium" title={hand.receiptId}>
                        #{String(hand.handNumber)}
                      </td>
                      <td className="px-3 py-3 text-[10px] text-[var(--muted)]">{formatLevel(hand.levelId)}</td>
                      <td className="px-3 py-3 text-[10px] text-[var(--muted)]">{t("stats.playerCount", { count: hand.players })}</td>
                      <td className={`px-3 py-3 text-[11px] font-semibold tabular-nums ${positive ? "text-[#327844]" : "text-[#b4443e]"}`}>
                        <span className="inline-flex items-center gap-1">
                          <ResultIcon className="size-3" /> {formatSignedTokens(hand.delta)}
                        </span>
                      </td>
                      <td className={`px-3 py-3 text-[10px] ${hand.archived ? "text-[#4b7652]" : "text-[#9a6c22]"}`}>
                        {t(hand.archived ? "stats.archived" : "stats.notArchived")}
                      </td>
                      <td className="px-5 py-3 text-right text-[9px] text-[var(--muted-light)]">
            ...
            <div className="px-5 py-14 text-center">
              ...
              <p className="mt-2 text-[10px] text-[var(--muted-light)]">{t("stats.emptyDescription")}</p>
            </div>
          )}
        </div>
      </div>
