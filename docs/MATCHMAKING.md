...
`node scripts/verify-p2p-hand.mjs --without-gossip` starts a real Rendezvous sidecar and two player sidecars that do not know each other's address. The guest deliberately drops all room-topic Gossipsub publications, so admission must use the signed request-response path instead of succeeding by accident through eventual gossip. The test verifies discovery, reliable admission, identical table IDs, distinct seats, and a complete hand.

`node scripts/verify-p2p-hand.mjs --relay --without-gossip` repeats the same fault-injected session with both players reserving a Circuit Relay path. It verifies that matchmaking survives NAT-style relay topology while direct connection upgrades remain optional.
